...
def test_sharded_data_loading_model_axis_2():
    ...
    with mesh:
        ...
        for batch in batches:
            ...
            localized = pjit(lambda x: x, in_axis_resources=batch.mesh_axes, out_axis_resources=PartitionSpec(None, None, None))(batch)
            shard_i: Shard
...
def test_sharded_data_loading_model_axis_2_not_microbatched():
    ...
    with mesh:
        ...
        for batch in batches:
            assert batch.shape == dataset.batch_shape()
            localized = pjit(lambda x: x, in_axis_resources=batch.mesh_axes, out_axis_resources=PartitionSpec(None, None))(batch)
            shard_i: Shard
            for i, shard_i in enumerate(batch.global_shards):
                data_axis_pos_i = shard_i.device.id // mesh_info.model_axis_size
                for j, shard_j in enumerate(batch.global_shards):
