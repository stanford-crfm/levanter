...
from levanter.optim.config import AdamConfig, SkipStepConfig, SkipStepState, OptimizerConfig
...
def test_update_fn_skip_logic(initialized_wrapped_optimizer, trigger_high_loss):
    ...
    state_after_population = populate_history(wrapped_optimizer, initial_state, params, min_data_points, base_loss, base_grads)
    ...
    chex.assert_trees_all_close(updates, expected_zero_updates, atol=1e-5,
                                err_msg="Updates were not zeroed out when step was skipped.")

    # Assert inner optimizer state is preserved
    chex.assert_trees_all_close(new_state.inner_opt_state, pre_skip_inner_opt_state,
                                err_msg="Inner optimizer state was modified during a skipped step.")
...
def test_update_fn_circular_buffer_and_count_cap(initialized_wrapped_optimizer):
    ...
    actual_losses_ordered = jnp.concatenate([current_state.losses[current_state.current_idx:], current_state.losses[:current_state.current_idx]])
    actual_gnorms_ordered = jnp.concatenate([current_state.grad_norms[current_state.current_idx:], current_state.grad_norms[:current_state.current_idx]])
...
def test_integration_with_optimizer_config_build():
    ...
    chex.assert_trees_all_close(updates, expected_zero_updates, atol=1e-7,
                                err_msg="Updates were not zeroed out when step was skipped in integration test.")

    # Assert inner optimizer state is preserved
    chex.assert_trees_all_close(final_state.inner_opt_state, pre_skip_inner_opt_state,
                                err_msg="Inner optimizer state was modified during a skipped step in integration test.")
