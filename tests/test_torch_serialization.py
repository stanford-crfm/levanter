...
def test_unflatten_linear_layer(out_dims_first: bool):
    H = hax.Axis("H", 10)
    W = hax.Axis("W", 20)
    D = hax.Axis("D", 30)
    B = hax.Axis("B", 40)
    linear = hax.nn.Linear((H, W), (D, B), key=jax.random.PRNGKey(0), use_bias=True)

    assert linear.weight.axes == (H, W, D, B)

    # first flatten the weight matrix
    flat = flatten_linear_layer(None, linear, out_dims_first_in_dict=out_dims_first)
    if out_dims_first:
    ...
    # now unflatten it
    unflat_dict = unflatten_linear_layer(None, flat, linear, out_dims_first_in_dict=out_dims_first)
