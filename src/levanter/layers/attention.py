...
def _te_flash_attention(
    ...
    *,
    ...
    q_class, k_class, v_class = _bin_and_group_axes_by_function(
        query, key, value, QPos, KPos, Key
    )
    ...
    if B != Bk:
        raise ValueError(
            f"Batch axes must be the same for q, k, and v: {q_class['B']} != {k_class['B']}"
        )

    if D != Dk:
        raise ValueError(
            f"Embedding axes must be the same for q, k, and v: {q_class['D']} != {k_class['D']}"
        )
    ...
    if bias:
        raise NotImplementedError(
            "Using bias with flash attention on GPU is not currently implemented."
        )
    ...
    attn_output = attn_output.rearrange(reference_out_shape.axes).astype(
        reference_out_shape.dtype
    )
...
def _bin_and_group_axes_by_function(q, k, v, QPos, KPos, Key):
    ...
    batch_axes = [
        ax for ax in q.axes if ax.name not in spoken_for and ax.name in present_in_all
    ]
...
class AttentionMask(eqx.Module):
    ...
    def materialize(
        ...
        if self.segment_ids is not None:
            segment_mask = _materialize_segment_mask(
                self.segment_ids, QPos, KPos, q_slice, k_slice
            )
    ...
    def __and__(self, other) -> "AttentionMask":
        ...
        return AttentionMask(
            is_causal=is_causal, explicit_mask=explicit_mask, segment_ids=segment_ids
        )
...
def _try_tpu_splash_attention(
    ...
    *,
    ...
    if dropout != 0.0:
        ...
        warnings.warn(
            "Splash attention does not support. Falling back to the reference implementation."
        )
        return None
    ...
    if bias is not None:
        ...
        warnings.warn(
            "Splash attention does not support bias. Falling back to the reference implementation."
        )
        return None
    ...
    try:
        ...
        warnings.warn(
            f"Could not use splash attention: {message}. Falling back to the reference"
        )
        return None
...
def _tpu_splash_attention(
    ...
    *,
    ...
    q_: jax.Array = _reshape_axes_for_bshd_bins(
        query, q_class, output_order=list("BHSD")
    ).array
    ...
    if Sk % 128 != 0:
        raise NotImplementedError(
            "Splash attention requires KPos to be a multiple of 128"
        )

    if block_size is not None and block_size % 128 != 0:
        raise NotImplementedError(
            f"Splash attention requires block_size to be a multiple of 128, got {block_size}"
        )
    ...
    def _physical_axis_for_binning(d):
        ...
        b_out = flatten(
            tuple(ax for ax in pspec_for_axis(d["B"]) if ax is not None) or None
        )
        h_out = flatten(
            tuple(ax for ax in pspec_for_axis(d["H"]) if ax is not None) or None
        )
        s_out = flatten(
            tuple(ax for ax in pspec_for_axis(d["S"]) if ax is not None) or None
        )
        d_out = flatten(
            tuple(ax for ax in pspec_for_axis(d["D"]) if ax is not None) or None
        )
    ...
    physical_axes_segments = (
        pspec_for_axis(segment_ids.axes) if segment_ids is not None else None
    )
    # do we have a batch axis in segment_ids? (needed for vmap below)
    if segment_ids is not None:
        index_of_seq_dim = segment_ids.axes.index(QPos)
        other_indices = [
            i for i in range(len(segment_ids.axes)) if i != index_of_seq_dim
        ]
    ...
    def wrap_flash_attention(q, k, v, segment_ids):
        ...
        if mask is None:
            ...
            if mask.explicit_mask is not None:
                raise NotImplementedError(
                    "Explicit masks are not yet supported for splash attention"
                )

        elif isinstance(mask, NamedArray):
            raise NotImplementedError(
                "NamedArray masks are not yet supported for splash attention"
            )
        else:
            raise ValueError(f"Unknown mask type: {mask}")

        kernel_mask = splash_attention_mask.MultiHeadMask(
            masks=[base_mask for _ in range(Hq)]
        )
...
class Attention(eqx.Module):
    ...
    def init(config: AttentionConfig, *, key) -> "Attention":
        ...
        rot_embs = (
            config.rope.build(config.HeadSize) if config.rope is not None else None
        )

        return Attention(
            config, q_proj, k_proj, v_proj, o_proj, q_norm, k_norm, rot_embs
        )
    ...
    def __call__(
        ...
        *,
        ...
        attn_output = attn_output.flatten_axes(
            ("kv_heads", "q_heads_per_group"), "heads"
        )
