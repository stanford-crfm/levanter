...
class PageTable(eqx.Module):
    ...
    def allocate_for_seqs(
        self, updated_seqs: jnp.ndarray | NamedArray, new_token_counts: NamedArray
        ...
        """
        if not isinstance(new_token_counts, NamedArray):
            raise ValueError("new_token_counts must be a NamedArray")

        if isinstance(updated_seqs, NamedArray):
            updated_seqs = updated_seqs.array

        # convert NamedArrays to plain arrays for easier manipulation inside jit/loops
        page_indices = self.page_indices.array
        page_owners = self.page_owners.array
        seq_lens = self.seq_lens.array

        padded_updated_seqs = jnp.where(updated_seqs < 0, self.max_seqs, updated_seqs)
        current_lens = jnp.where(seq_lens < 0, 0, seq_lens)
        new_lens_tmp = current_lens.at[padded_updated_seqs].add(new_token_counts.array, mode="drop")
        new_lens = jnp.where(seq_lens < 0, jnp.where(new_lens_tmp > 0, new_lens_tmp, -1), new_lens_tmp)
        ...
        def _alloc_one_page(_, state):
            seq_id, page_idx_for_seq, page_indices, page_owners = state
            free_page_idx = jnp.argmin(page_owners)
            free_page_idx = eqx.error_if(free_page_idx, page_owners[free_page_idx] != -1, "No free pages")
            page_owners = page_owners.at[free_page_idx].set(seq_id)
            page_indices = page_indices.at[seq_id, page_idx_for_seq].set(free_page_idx)
            return seq_id, page_idx_for_seq + 1, page_indices, page_owners

        def _alloc_pages_for_seq(seq_id, carry):
            page_indices, page_owners = carry
            num_needed = new_num_pages_needed[seq_id]
            old_needed = old_num_pages_needed[seq_id]

            def body(page_idx, state):
                _, _, pi, po = _alloc_one_page(None, (seq_id, page_idx, state[0], state[1]))
                return pi, po

            page_indices, page_owners = jax.lax.fori_loop(old_needed, num_needed, body, (page_indices, page_owners))
            return page_indices, page_owners

        page_indices, page_owners = jax.lax.fori_loop(0, self.max_seqs, _alloc_pages_for_seq, (page_indices, page_owners))

        new_table = dataclasses.replace(
            self,
            page_indices=hax.named(page_indices, self.page_indices.axes),
            page_owners=hax.named(page_owners, self.page_owners.axes),
            seq_lens=hax.named(new_lens, self.seq_lens.axes),
        )
    ...
    def _slice_batch_info(self, updated_seqs, old_seq_lens, new_table, new_token_counts):
        axis = new_table.seq_lens.axes[0]
        updated_named = hax.named(updated_seqs, axis)
        mask = updated_named >= 0
        safe_updated = hax.where(mask, updated_named, 0)

        page_indices = new_table.page_indices["seq", safe_updated]
        mask_b = hax.broadcast_to(mask, page_indices.axes)
        page_indices = hax.where(mask_b, page_indices, -1)
        ...
        old_seq_lens = old_seq_lens["seq", safe_updated]
        ...
        new_token_dests = hax.named(jnp.full_like(new_token_counts.array, -1, dtype=jnp.int32), new_token_counts.axes)
