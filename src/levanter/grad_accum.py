...
def accumulate_gradients_sharded(
    ...
    Accumulate gradients across a sharded batch, keeping a local copy of the gradient on each row of the data
     ...
     Parameters:
        ...
        Batch: the axis along which the batch is sharded
    ...
    def accum_f(model: M, *args: X, **kwargs) -> Tuple[float, M]:
        should_batch_spec = _instantiate_arg_specs_for_args(batched_args, batched_kwargs, args, kwargs)
        ...
        def loop(acc, *mbatch_args, **mbatch_kwargs):
            loss, grad = acc
        ...
        loss, grad = hax.fold(loop, AccumStep, is_scanned=should_batch_spec)((loss, grad), *args, **kwargs)
...
# TODO: would be better to use inspect
def _instantiate_arg_specs_for_args(batched_args, batched_kwargs, args, kwargs) -> Tuple[Tuple[bool], Dict[str, bool]]:
    """Expands the provided batched_args and batched_kwargs into bool tuples with the same shape as arg and /kwargs."""
    ...
    return batched_args_spec, batched_kwarg_spec  # type: ignore
