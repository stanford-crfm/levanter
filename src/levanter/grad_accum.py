    ...
    *inputs: X,
    ...
    Accumulate gradients across a sharded dataset, keeping a local copy of the gradient on each row of the data
     ...
     Parameters:
        ...
        model: the model we're computing the gradient of
    ...
    if data_axis_size is None or physical_axis_name is None:
    ...
    loss, grad = jnp.zeros(()), jax.tree_util.tree_map(jnp.zeros_like, model)

    # second, we want to reshape our data to (num_micro_steps, micro_batch_size, ...), sharded along the data axis
    inputs = _reshape_for_microbatch(Batch, Microbatch, AccumStep, compute_axis_mapping, inputs)

    # third, we want to do compute. we do this in a loop that's wrapped in a fold
    with hax.axis_mapping(compute_axis_mapping):
        ...
        def loop(acc, microbatch):
            ...
            return loss + this_loss, jax.tree_map(jnp.add, grad, this_grad)
...
@named_call
def _reshape_for_microbatch(Batch: Axis, Microbatch: Axis, AccumStep: Axis, axis_mapping, inputs):
    def _reshape(x):
        if isinstance(x, hax.NamedArray):
            x = x.unflatten_axis(Batch, (AccumStep, Microbatch))
            return shard_with_axis_mapping(x, axis_mapping)
