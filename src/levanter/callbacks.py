...
def log_step_info(step: StepInfo):
    levanter.log_metrics({"train/loss": step.loss, "global_step": step.step}, step=step.step)
...
def log_performance_stats(
    ...
    def log_performance_stats(step_info: StepInfo):
        ...
        levanter.log_metrics({wrap_key("total_tokens"): total_tokens}, step=step_info.step)

        if flops_per_example:
            total_flops = flops_per_example * batch_size * step_info.step
            levanter.log_metrics({wrap_key("total_gflops"): total_flops / 1e9}, step=step_info.step)

        if step_info.step_duration != 0.0:
            levanter.log_metrics(
                {
...
def log_memory_usage(sample_interval: float = 1.0, log_individual_devices: bool = False):
    ...
    def log_memory_usage(step: StepInfo):
        ...
        if match:
            ...
            levanter.log_metrics({"memory/total": memory_usage / 1e6}, step=step.step)
        ...
        if log_individual_devices:
            ...
            for match in regex.finditer(per_device):
                ...
                levanter.log_metrics({f"memory/device/{device_name}": memory_usage / 1e6}, step=step.step)
        ...
        for match in regex.finditer(by_kind):
            ...
            levanter.log_metrics({f"memory/{match.group(3)}": memory_usage / 1e6}, step=step.step)
