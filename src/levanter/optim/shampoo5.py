...
@OptimizerConfig.register_subclass("shampoo5")
@dataclass
class Shampoo5Config(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_shampoo5(
                    b1=self.momentum,
...
def scale_by_shampoo5(
    ...
    def init_fn(params: Updates) -> dict:
        ...
        exp_avg_adam = otu.tree_zeros_like(params, dtype=mu_dtype)
    ...
    def update_step(updates: Updates, state: dict, scanned_layers_: Updates) -> tuple[Updates, dict]:
        ...
        exp_avg_adam = state["exp_avg_adam"]
        exp_avg_sq = state["exp_avg_sq"]
        exp_avg_adam = jax.tree.map(lambda m, g: None if g is None else b1 * m + (1 - b1) * g, exp_avg_adam, updates)
        ...
        if partition_grads_into_blocks:
            ...
            blocked_exp_avg_adam = jax.tree.map(
                lambda g, p_cls, s: _map_fn(False, 0, int(s), p_cls.partition, g),
                exp_avg_adam,
                partitioners,
                scanned_layers_,
            ...
            blocked_exp_avg_adam = exp_avg_adam
        ...
        blocked_norm_updates_leaves = jax.tree.map(lambda g: g * jnp.sqrt(1 - b2 ** state["count"]) / jnp.sqrt(1 - b2), blocked_norm_updates_leaves)

        blocked_exp_avg_leaves = jax.tree.map(
            lambda _, nm, e, u: _map_fn(False, 0, nm, lambda e, u: b1 * e + (1 - b1) * u, e, u),
            jax.tree.leaves(dummy_updates_tree),
            jax.tree.leaves(n_dims_to_map),
            jax.tree.leaves(blocked_exp_avg),
            blocked_norm_updates_leaves
        )

        blocked_exp_avg_leaves = jax.tree.map(lambda g: g / ((1 - b1 ** state["count"])) , blocked_exp_avg_leaves)
        ...
        blocked_adam_updates_leaves = jax.tree.map(
            ...
            jax.tree.leaves(dummy_updates_tree),
            jax.tree.leaves(n_dims_to_map),
            jax.tree.leaves(blocked_exp_avg_adam),
        ...
        blocked_norm_updates_leaves = jax.tree.map(
            ...
            jax.tree.leaves(dummy_updates_tree),
            jax.tree.leaves(n_dims_to_map),
            jax.tree.leaves(blocked_exp_avg_leaves),
        ...
        if partition_grads_into_blocks:
            ...
            exp_avg_adam = blocked_exp_avg_adam
        # unmerge
        if merge_small_dims:
            norm_updates = jax.tree.map(
        ...
        exp_avg_adam = otu.tree_cast(exp_avg_adam, mu_dtype)
        new_state = {
            "GG": new_GG,
            "exp_avg": exp_avg,
            "exp_avg_sq": exp_avg_sq,
