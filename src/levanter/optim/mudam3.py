...
@OptimizerConfig.register_subclass("mudam3")
@dataclass
class Mudam3Config(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_mudam3(
                    b1=self.momentum,
...
def scale_by_mudam3(
    ...
    def update_step(updates: Updates, state: dict, scanned_layers_: Updates) -> tuple[Updates, dict]:
        ...
        blocked_norm_updates_leaves = jax.tree.map(
            lambda _, nm, e, gg: _map_fn(False, 0, nm, partial(ns_generalized, precision=precision, b2=b2, eps=epsilon), e, gg),
...
def matrix_inverse_sqrt(matrix: jnp.ndarray) -> jnp.ndarray:
  ...
  Y = matrix / alpha
  Z = jnp.eye(matrix.shape[0])
  for i in range(10):
    T = 0.5 * ( 3  * jnp.eye(matrix.shape[0]) - Z @ Y )
    Y = Y @ T
    Z = T @ Z
  return Z / jnp.sqrt(alpha)
...
def ns_generalized(
    ...
    P = GG[idx] * b2
    coeffs = jnp.array([
        [3.6,        -6262/1024,  3090/1024],
        [3.6,        -6869/1024,  3716/1024],
        [3.6,        -7539/1024,  4477/1024],
        [3.6,        -8275/1024,  5393/1024],
        [3.6,        -6168/1024,  3733/1024],
        [3.6/2.04,   -2170/(1024*2.04), 570/(1024*2.04)],
    ])  
