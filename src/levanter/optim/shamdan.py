...
@OptimizerConfig.register_subclass("shamdan")
@dataclass
class ShamdanConfig(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_shamdan(
                    b1=self.momentum,
...
def scale_by_shamdan(
    ...
    def update_step(updates: Updates, state: dict, scanned_layers_: Updates) -> tuple[Updates, dict]:
        ...
        blocked_norm_updates_leaves = jax.tree.map(
            lambda _, nm, e, gg: _map_fn(False, 0, nm, partial(shamdan_update, precision=precision, steps=steps, eps=epsilon, b2=b2), e, gg),
        ...
        blocked_norm_updates = jax.tree.map(lambda g: g * jnp.sqrt(1 - b2 ** state["count"]) / (jnp.sqrt(1 - b2) *(1 - b1 ** state["count"])) , blocked_norm_updates)
