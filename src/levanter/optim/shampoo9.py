...
@OptimizerConfig.register_subclass("shampoo9")
@dataclass
class Shampoo9Config(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_shampoo9(
                    b1=self.momentum,
...
def shampoo_update(
    ...
    for i in range(steps):
        A = X_copy @ X_copy.T 
        B = b * A + c * A @ A
        X_copy = a * X_copy + B @ X_copy
    X = (X / jnp.linalg.norm(X)) * jnp.linalg.norm(X_copy)
    if transpose:
        X = X.T
