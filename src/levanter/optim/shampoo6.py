...
@OptimizerConfig.register_subclass("shampoo6")
@dataclass
class Shampoo6Config(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_shampoo6(
                    b1=self.momentum,
...
def shampoo_update(
    ...
    if idx == 1:
        ...
        transpose = True
    P_diag = jnp.diag(P)
    X_diag = jnp.linalg.norm(X, axis=1)
    row_scaled = jnp.diag(jnp.sqrt(P.shape[0]) * X_diag / jnp.sqrt(P_diag))
    P_over_X_factor = 2.0
    X = X / P_over_X_factor
    P = P - X @ X.T
