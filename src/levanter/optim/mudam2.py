...
@OptimizerConfig.register_subclass("mudam2")
@dataclass
class Mudam2Config(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_mudam2(
                    b1=self.momentum,
...
def scale_by_mudam2(
    ...
    def update_step(updates: Updates, state: dict, scanned_layers_: Updates) -> tuple[Updates, dict]:
        ...
        blocked_norm_updates_leaves = jax.tree.map(
            lambda _, nm, e, gg: _map_fn(False, 0, nm, partial(ns_generalized, precision=precision, b2=b2, steps=steps, eps=epsilon), e, gg),
