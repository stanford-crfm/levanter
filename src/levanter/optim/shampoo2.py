...
@OptimizerConfig.register_subclass("shampoo2")
@dataclass
class Shampoo2Config(OptimizerConfig):
    ...
    def build(self, num_train_steps):
        ...
        def optimizer(learning_rate, adam_lr):
            ...
            def muon_transform():
                components = []
                components.append(scale_by_shampoo2(
                    b1=self.momentum,
...
def scale_by_shampoo2(
    ...
    def update_step(updates: Updates, state: dict, scanned_layers_: Updates) -> tuple[Updates, dict]:
        ...
        blocked_norm_updates = jax.tree.map(lambda g: g * jnp.sqrt(1 - b2 ** state["count"]) / (1 - b1 ** state["count"]), blocked_norm_updates)
...
def matrix_inverse_sqrt(matrix: jnp.ndarray) -> jnp.ndarray:
  ...
  for i in range(10):
    T = 0.5 * (3 * jnp.eye(matrix.shape[0]) - Y @ Z)
    Y = T @ Y 
    Z = Z @ T
  return Z
