        ...
        try:
            try:
                ...
                if "preempted" in str(e):
                    logger.warning("preempted", exc_info=True)
                    return TpuPreempted()
                else:
                    logger.exception("tpu job failed")
    ...
    while num_failures < max_retries_failure and num_preemptions < max_retries_preemption:
        try:
            ...
            match out:
                ...
                case TpuPreempted():
                    num_preemptions += 1
                    logger.warning(f"Preempted {num_preemptions} times")
                case TpuFailed():
                    num_preemptions += 1
...
def run_docker_on_pod(image_id, command, *, tpu_type, env, name="levanter"):
