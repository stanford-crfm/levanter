...
from levanter.ray_tpu.src.ray_tpu import RayTpuManager

logger = logging.getLogger("ray")

_tpu_manager = RayTpuManager()
...
def run_on_pod(remote_fn: RemoteFunction | Callable, tpu_type: str, num_slices: int=1) -> ray.ObjectRef:
    """
    ...
    def do_run(remote_fn) -> _TpuRunResult:
        ...
        handles = _tpu_manager.remote(remote_fn, topology={tpu_type: num_slices}, multislice=True)
        info = _TpuInfo(tpu_type, "ACTIVE", "TPU")
...
def run_on_pod_resumable(remote_fn, tpu_type, num_slices=1, max_retries_preemption=1e6, max_retries_failure=10):
    """
    ...
    while num_failures < max_retries_failure and num_preemptions < max_retries_preemption:
        ...
        attempt += 1
        problem = None
        future = run_on_pod(remote_fn, tpu_type, num_slices)
        try:
            out = ray.get(future)
        except (ray.exceptions.ActorUnavailableError, ray.exceptions.ActorDiedError) as e:
            problem = e
            num_preemptions += 1
            ...
            problem = e
            if isinstance(e, ray.exceptions.RayTaskError) and "preempted" in str(e).lower():
                num_preemptions += 1
                logger.warning(f"Preempted {num_preemptions} times, {e}")
            else:
                num_failures += 1
...
def run_on_pod_multislice_resumable(remote_fn, tpu_type, num_slices=1, max_retries_preemption=1e6, max_retries_failure=10):
