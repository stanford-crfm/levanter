...
def gpt2_attention_init(in_dim, num_heads, head_dim, dropout_prob, *, key)->Gpt2Attention:
    attn = Gpt2Attention(in_dim, num_heads=num_heads, head_dim=head_dim, dropout_prob=dropout_prob, key=key, causal=True)  # type: ignore
    #
...
class PjitGpt2Block(eqx.Module):
    ...
    def __init__(self,
        ...
        self.attn = gpt2_attention_init(hidden_size, config.num_heads, head_dim, config.attn_pdrop, key=k_attn)
